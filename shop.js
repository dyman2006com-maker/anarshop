...
function initAuthUI(){
  ...
  document.querySelectorAll('a.um-item').forEach(a => {
    const t = a.textContent.trim();
    if(t === 'پروفایل من') a.href = 'profile.html';
    if(t === 'سفارش‌های من') a.href = 'profile.html';
  });
}
...
function initMobile(){
  ...
  nav.innerHTML = `
    ...
    <a class="m-item" href="${u?'profile.html':'auth.html'}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>${u?'حساب من':'ورود'}</a>
  `;
